...
class Address:
    ...
    def __lt__(self, other):
        assert(isinstance(other, Address))
        return eth_utils.to_normalized_address(self.address) < eth_utils.to_normalized_address(other.address)
...
class Contract:
    ...
    def _wait_for_receipt(self, web3, transaction_hash, timeout=300):
        st = time.time()
        while True:
    ...
    def _transact(self, web3, log_message, func):
        try:
            ...
            tx_hash = func()
            self.logger.info(f"Transaction {tx_hash} submitted")
    ...
    def _prepare_receipt(self, web3, transaction_hash):
        ...
        if receipt is None:
            return None
        return Receipt(transaction_hash=transaction_hash, transfers=[])
...
class Receipt:
    ...
    Attributes:
        ...
        transfers: A list of ERC20 token transfers resulting from the execution
            ...
            `Transfer` class.
    """
    def __init__(self, transaction_hash: bytes, transfers: list):
        assert(isinstance(transaction_hash, bytes))
