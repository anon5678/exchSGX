...
std::string CTxIn::ToString() const
{
    ...
    if (prevout.IsNull())
        ...
        str += strprintf(", scriptSig=%s", HexStr(scriptSig).substr(0, 24).c_str());
...
{
    return strprintf("CTxOut(nValue=%d.%08d, scriptPubKey=%s)", nValue / COIN, nValue % COIN, HexStr(scriptPubKey).substr(0, 30).c_str());
}
