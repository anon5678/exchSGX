...
public:
    ...
    void read(char* pch, size_t nSize)
    {
        ...
        if (nReadPosNext >= vch.size())
        {
            if (nReadPosNext > vch.size())
            {
                throw std::runtime_error("CDataStream::read(): end of data");
            }
    ...
    void ignore(int nSize)
    {
        ...
        if (nSize < 0) {
            throw std::runtime_error("CDataStream::ignore(): nSize negative");
        }
        unsigned int nReadPosNext = nReadPos + nSize;
        if (nReadPosNext >= vch.size())
        {
            if (nReadPosNext > vch.size())
                throw std::runtime_error("CDataStream::ignore(): end of data");
            nReadPos = 0;
            vch.clear();
            return;
        }
