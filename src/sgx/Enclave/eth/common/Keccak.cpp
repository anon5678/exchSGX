...
Keccak::Keccak(Bits bits)
        : m_blockSize(200 - 2 * ((unsigned int)bits / 8)),
          m_bits(bits)
{
    reset();
}
...
{
    unsigned int blockSize = 200 - 2 * ((unsigned int)m_bits / 8);
    // add padding
...
std::string Keccak::getHash()
{
    ...
    unsigned int hashLength = (unsigned int)m_bits / 64;
    std::string result;
    ...
    unsigned int remainder = (unsigned int)m_bits - hashLength * 64;
