...
void recursiveMerk(const arrdigest *level, int size, int path) {
  // k = floor(size/2)
  int k = (size + (size & 1)) / 2;
  // build next (upper) level of the tree with half nodes
  ...
  for (int i = 0; i < k; ++i) {
    ...
    // right node is 2*i + 1 unless missing
    const unsigned char *right_node =
        ((2 * i + 1) == size ? left_node : level[2 * i + 1]);
    sha256double(left_node, right_node, next[i]);
    // if the path go right, then output the left node
    // as we need that for verification
    if (path == (2 * i + 1)) {
      std::cout << "L: ";
      hexdump(left_node, SHA256_DIGEST_LENGTH);
      // std::cout << base64_encode(std::string(left_node, left_node + 32)) << std::endl;
      continue;
    }
    if (path == (2 * i)) {
      std::cout << "R: ";
      if (left_node != right_node) {
        ...
        // std::cout << base64_encode(std::string(right_node, right_node + 32)) << std::endl;
      } else
        std::cout << std::endl;
    }
  }
...
int main() {
  ...
  const std::string path1[3] = {"5uNkxNynhZ7LGFdwn/Ay2geQ2OIQJ7s7TrCJfBeagaA=",
